...
@section('title_page')
    Edit Sales Order - {{ $order->order_no }}
@endsection
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="card card-primary card-outline">
                        ...
                        <form method="post" action="{{ route('sales-orders.update', $order->id) }}" id="so-form">
                            @csrf
                            @method('PATCH')
                                        ...
                                        <div class="form-group row mb-2">
                                            ...
                                            <div class="col-sm-9">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="order_no" value="{{ old('order_no', $order->order_no) }}"
                                        ...
                                        <div class="form-group row mb-2">
                                            ...
                                            <div class="col-sm-9">
                                                <div class="input-group input-group-sm">
                                                    ...
                                                    <input type="text" name="reference_no" value="{{ old('reference_no', $order->reference_no) }}"
                                                ...
                                                <select name="company_entity_id" id="company_entity_id"
                                                    ...
                                                    @foreach ($entities as $entity)
                                                        <option value="{{ $entity->id }}"
                                                            {{ old('company_entity_id', $order->company_entity_id) == $entity->id ? 'selected' : '' }}>
                                            ...
                                            <div class="col-sm-9">
                                                <select name="business_partner_id" id="business_partner_id"
                                                    ...
                                                    @foreach ($customers as $c)
                                                        <option value="{{ $c->id }}"
                                                            {{ old('business_partner_id', $order->business_partner_id) == $c->id ? 'selected' : '' }}
                                            ...
                                            <div class="col-sm-9">
                                                <select name="warehouse_id" class="form-control form-control-sm select2bs4"
                                                    ...
                                                    @foreach ($warehouses as $w)
                                                        <option value="{{ $w->id }}"
                                                            {{ old('warehouse_id', $order->warehouse_id) == $w->id ? 'selected' : '' }}>
                                                            {{ $w->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select name="currency_id" id="currency_id"
                                                    ...
                                                    @foreach ($currencies as $currency)
                                                        <option value="{{ $currency->id }}"
                                                            {{ old('currency_id', $order->currency_id) == $currency->id ? 'selected' : '' }}>
                                            ...
                                            <div class="col-sm-9">
                                                <div class="input-group input-group-sm">
                                                    <input type="number" name="exchange_rate" id="exchange_rate"
                                                        ...
                                                        value="{{ old('exchange_rate', $order->exchange_rate) }}">
                                        ...
                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="form-group row mb-2">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <textarea name="delivery_address" id="delivery_address" class="form-control form-control-sm" rows="3">{{ old('delivery_address', $order->delivery_address) }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group row mb-2">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <input type="text" name="delivery_contact_person" id="delivery_contact_person" value="{{ old('delivery_contact_person', $order->delivery_contact_person) }}" class="form-control form-control-sm">
                                                    </div>
                                                </div>
                                                <div class="form-group row mb-2">
                                                    <label class="col-sm-4 col-form-label">Phone</label>
                                                    <div class="col-sm-8">
                                                        <input type="text" name="delivery_phone" id="delivery_phone" value="{{ old('delivery_phone', $order->delivery_phone) }}" class="form-control form-control-sm">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="card-footer">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <a href="{{ route('sales-orders.show', $order->id) }}" class="btn btn-default">
        ...
        $(document).ready(function() {
            ...
            function loadCustomerProjects(bpId, selectedId) {
                ...
                $.get("{{ route('business_partners.projects.list') }}", { business_partner_id: bpId }, function(data) {
                    ...
                    $select.val(selectedId || "{{ old('business_partner_project_id', $order->business_partner_project_id) }}").trigger('change');
                });
            }
            ...
            @if ($order->business_partner_id)
            loadCustomerProjects("{{ $order->business_partner_id }}", "{{ old('business_partner_project_id', $order->business_partner_project_id) }}");
            @endif
            ...
            // Load existing lines
            if (window.prefill && window.prefill.lines && window.prefill.lines.length > 0) {
                $tb.empty();
                i = 0;
            ...
            function addLineRow(data = {}) {
                ...
                const partNumberSelect = $(tr).find('.part-number-select');
                if (orderType === 'item' && data.item_id) {
                    const item = window.inventoryItems.find(i => i.id == data.item_id);
                    if (item && item.part_numbers && item.part_numbers.length) {
                        partNumberSelect.empty().append('<option value="">Internal code</option>');
                        item.part_numbers.forEach(function(pn) {
                            partNumberSelect.append(`<option value="${pn.id}" ${pn.id == data.part_number_id ? 'selected' : ''}>${pn.part_number}${pn.description ? ' - ' + pn.description : ''}</option>`);
                        });
