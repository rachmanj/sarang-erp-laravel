...
@section('title', 'Edit GR/GI Document')

@section('title_page')
    Edit {{ $grGi->document_number }}
@endsection

@section('breadcrumb_title')
    <li class="breadcrumb-item"><a href="/dashboard">Dashboard</a></li>
    <li class="breadcrumb-item"><a href="{{ route('gr-gi.index') }}">GR/GI Management</a></li>
    <li class="breadcrumb-item"><a href="{{ route('gr-gi.show', $grGi) }}">{{ $grGi->document_number }}</a></li>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('gr-gi.show', $grGi) }}" class="btn btn-tool btn-sm">
                                    <i class="fas fa-arrow-left"></i> Back to Document
                                </a>
                            </div>
                        </div>
                        <form action="{{ route('gr-gi.update', $grGi) }}" method="POST" id="gr-gi-form">
                            @csrf
                            @method('PATCH')
                            <div class="card-body">
                                <!-- Header Information -->
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control"
                                                value="{{ $grGi->document_type_name }}" readonly>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="purpose_id" id="purpose_id" required>
                                                ...
                                                @foreach ($purposes as $purpose)
                                                    <option value="{{ $purpose->id }}"
                                                        {{ $grGi->purpose_id == $purpose->id ? 'selected' : '' }}>
                                                        {{ $purpose->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            ...
                                            <select class="form-control" name="warehouse_id" id="warehouse_id" required>
                                                ...
                                                @foreach ($warehouses as $warehouse)
                                                    <option value="{{ $warehouse->id }}"
                                                        {{ $grGi->warehouse_id == $warehouse->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="date" class="form-control" name="transaction_date"
                                                id="transaction_date"
                                                value="{{ old('transaction_date', $grGi->transaction_date->format('Y-m-d')) }}"
                                                required>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="reference_number"
                                                id="reference_number"
                                                value="{{ old('reference_number', $grGi->reference_number) }}"
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" name="notes" id="notes" rows="2" placeholder="Optional notes">{{ old('notes', $grGi->notes) }}</textarea>
                                            @error('notes')
                                                <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered" id="lines-table">
                                        ...
                                        <tbody id="lines-tbody">
                                            @foreach ($grGi->lines as $index => $line)
                                                <tr class="line-row">
                                                    <td>
                                                        <select class="form-control item-select"
                                                            name="lines[{{ $index }}][item_id]" required>
                                                    ...
                                                    <td>
                                                        <input type="number" class="form-control quantity-input"
                                                            name="lines[{{ $index }}][quantity]" step="0.001"
                                                            min="0.001" value="{{ $line->quantity }}" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" class="form-control unit-price-input"
                                                            name="lines[{{ $index }}][unit_price]" step="0.01"
                                                            min="0" value="{{ $line->unit_price }}" required>
                                                    </td>
                                                    <td>
                                                        <input type="number" class="form-control total-amount-input"
                                                            value="{{ $line->total_amount }}" readonly>
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control"
                                                            name="lines[{{ $index }}][notes]"
                                                            value="{{ $line->notes }}" placeholder="Optional notes">
                                                    </td>
                                                    <td>
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-sm">
                                                <tr>
                                                    ...
                                                    <td class="text-right">
                                                        <strong
                                                            id="total-amount">{{ number_format($grGi->total_amount, 2) }}</strong>
                                                    </td>
                                                </tr>
                                            </table>
                                        </div>
                            ...
                            <div class="card-footer">
                                ...
                                <a href="{{ route('gr-gi.show', $grGi) }}" class="btn btn-secondary">
...
@push('scripts')
    <script>
        $(document).ready(function() {
            let lineIndex = {{ $grGi->lines->count() - 1 }};
