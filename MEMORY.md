...
### [026] Multi-Entity Company Profile Foundation (2025-11-28) 🚧 IN PROGRESS
...
### [027] Entity-Aware Numbering & Service Update (2025-11-28) 🚧 IN PROGRESS

**Challenge**: After adding multi-entity schema, purchasing and sales processes still reused shared document numbers and controllers didn’t persist `company_entity_id`, making it impossible to segregate transactions per entity or render correct letterheads.
...
### [068] Inventory Low Stock & Valuation Report Stabilisation (2025-11-29) ✅ COMPLETE
...
### [068] Audit Trail System Analysis and Enhancement Plan (2025-01-20) ✅ COMPLETE
...
### [069] First Things to Do Manual Creation (2025-01-20) ✅ COMPLETE
...
**Key Learning**: ERP system requires systematic setup following specific order: company configuration → financial foundation → master data → business operations → user management. Critical dependencies exist (product categories must have account mappings before inventory items, business partners require account assignments, approval workflows need user roles). Manual provides comprehensive 4-6 hour setup guide with clear priorities, verification steps, and common issue resolution enabling successful system implementation.

### [070] Product Category Hierarchical UI Improvements (2025-01-21) ✅ COMPLETE
...
**Key Learning**: Phased migration approach minimizes risk while achieving complete system consistency. Entity resolution patterns vary by document type: inheritance for purchase/sales documents (PO→GRPO, SO→DO), chain resolution for asset documents (Asset→PurchaseInvoice), and default assignment for accounting documents (Cash Expense, Account Statement). Journal entity resolution requires sophisticated source type handling supporting polymorphic relationships. Complete migration eliminates dual-format complexity, enables unified entity-specific reporting, and simplifies sequence management with single format standard. Documentation must be comprehensive and bilingual for Indonesian businesses.

### [074] Direct Cash Purchase Feature Implementation (2025-12-26) ✅ COMPLETE

**Challenge**: Users frequently perform direct cash purchases (buying items with immediate cash payment) and prefer simplified workflow (Purchase Invoice → Post) instead of full PO → GRPO → PI → PP flow. System lacked support for direct cash purchases, automatic account selection for non-accounting users, and automatic inventory updates. Additionally, "Direct Purchase" checkbox overlapped with Payment Method selection, causing user confusion.
...
**Key Learning**: Direct cash purchases require different accounting flow (Inventory → Cash) than credit purchases (AP UnInvoice → Utang Dagang → Cash). Auto-setting `is_direct_purchase` based on payment method and PO/GRPO context simplifies UI and matches user expectations. Product category account mapping enables automatic account selection, reducing errors for non-accounting users. Multi-UOM support requires base quantity tracking and conversion factors. Unpost functionality must reverse journal entries and delete inventory transactions while maintaining data integrity. Cash account selection provides flexibility for multiple cash accounts while maintaining backward compatibility with default account. Removing redundant UI elements (checkbox) improves user experience when logic can be derived automatically.

### [075] GR/GI PostingService Integration & Model Namespace Fixes (2025-12-30) ✅ COMPLETE
...
### [077] Inventory-Warehouse Stock Accuracy Verification & Reconciliation (2026-02-03) ✅ COMPLETE
...
### [078] Warehouse Transfer Page Enhancement - Dedicated Page with Custom Autocomplete Search (2026-02-03) ✅ COMPLETE
...
### [079] Opening Balance Sales Invoices for Service Transactions (2026-02-04) ✅ COMPLETE
