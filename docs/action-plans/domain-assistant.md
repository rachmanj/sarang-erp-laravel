...
**Shipped same day (iterations):** Tools `search_sales_invoices` and `get_sales_invoice_detail` (header + `sales_invoice_lines`). Invoice **number** lookup uses `scopeActiveCompanyEntities` so invoices on non-default active entities are visible; list/browse without a number still uses default entity + `see-all-record-switch` / `show_all_records` as elsewhere. Prompt and tool descriptions steer “invoice / faktur” to AR tools, not sales orders. Navbar launcher **`fas fa-robot`**. HELP: `domain-assistant-manual-en.md` / `domain-assistant-manual-id.md`, `help-navigation.json` entry `domain-assistant` — run **`php artisan help:reindex`** after manual changes.
