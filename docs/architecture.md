...
-   **Sales Receipts**: Payment collection with invoice-first flow and explicit allocation (SR-YYYYMM-######). Select customer → load outstanding invoices via `getAvailableInvoices` (requires **`company_entity_id`**; filters `sales_invoices.company_entity_id`) → select invoices and allocation amounts → receipt lines auto-populated. **Rounding tolerance**: when actual cash received differs from allocation total within ERP parameter **`sales_receipt_rounding_tolerance`** (default Rp 999,999), the difference is stored as **`rounding_amount`** and posted to a user-selectable **Rounding Account** (default **`7.1.4` Selisih Pembulatan (Rounding)** via `rounding_account_id` ERP parameter). Per-invoice allocation still capped at remaining balance. Cash line amount is editable after auto-fill so users can enter rounded tender (e.g. invoice due Rp 8,245,999.99, receive Rp 8,246,000.00). **Store/update** reject allocations when invoice entity ≠ receipt company. Create/edit UI reloads invoices when **Company** changes. Mirrors Purchase Payment pattern. **Posting (current)**: `SalesReceiptJournalBuilder` + `CashJournalLineBuilder` debit cash/bank for **`total_amount`**, credit AR for **`total_amount − rounding_amount`**, and credit/debit rounding account for gain/loss when `rounding_amount ≠ 0`. **Legacy repair**: SRs posted before 2026-06-23 may have debited `1.1.1.01` Kas di Tangan regardless of selected bank — use `php artisan sales-receipts:repair-bank-journals` (`--dry-run`, `--receipt-no=`, `--id=`, `--force`).
...
-   **Purchase Payments**: Vendor payment processing with allocation (PP-YYYYMM-######). **Rounding tolerance** mirrors Sales Receipts via **`purchase_payment_rounding_tolerance`** and shared **`rounding_account_id`** ERP parameter; `PaymentRoundingService` resolves difference; `PurchasePaymentJournalBuilder` posts AP settle + rounding gain/loss (polarity inverted vs SR because cash is credited).
...
-   **Purchase Payment Accounting**: Debit Utang Dagang for **`total_amount − rounding_amount`**, Credit Cash/Bank for **`total_amount`** (from payment line `account_id`, grouped by COA account via `CashJournalLineBuilder`); when `rounding_amount > 0` (overpaid) debit rounding account (loss), when `< 0` credit rounding account (gain).
...
-   **Sales Receipt Accounting**: Debit Cash/Bank for **`total_amount`** (from receipt line `account_id`, grouped by COA account via `CashJournalLineBuilder`), Credit Piutang Dagang for **`total_amount − rounding_amount`**; rounding gain/loss line to **`rounding_account_id`** when `rounding_amount ≠ 0` (gain = credit rounding, loss = debit rounding). Journal entity resolved from `SalesReceipt.company_entity_id` via `PostingService::resolveJournalEntity()`. **`PaymentRoundingService`** centralizes tolerance check and default account resolution. **Legacy data**: pre-2026-06-23 postings used hard-coded `1.1.1.01` and often default entity (CV) — repair via `sales-receipts:repair-bank-journals` (reverse + repost).
