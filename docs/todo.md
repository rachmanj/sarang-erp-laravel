...
**Last Updated**: 2026-05-30 (AR list export, SR entity filter, cash expense date range + HELP docs)
...
-   `[done] P1: Purchase Order create/edit — totals + item search reliability [declare updatingHeaderDiscount before first updateTotals (fix TDZ ReferenceError on edit init); align edit updateTotals with create (header % vs amount, clear stale discount when 0%, Select2 VAT/WTax parsing); input+change on header discount; toastr @json session flashes; resources/views/purchase_orders/create.blade.php, edit.blade.php; docs: architecture, decisions, MEMORY [109], manuals purchase-module EN/ID, todo] (completed: 2026-05-04)`
...
-   `[done] P2: AR/AP list UX + HELP manuals [Sales Invoice list Export Excel (`sales-invoices.export`, SalesInvoiceListExport, SalesInvoiceListTest); Sales Receipt `getAvailableInvoices` + store/update filter by `company_entity_id` (SalesReceiptAvailableInvoicesTest); Cash Expenses list date range (daterangepicker + `from`/`to` on data, CashExpenseListTest); manuals cash-expense-*, sales-invoice/receipt manual updates, help-navigation.json; docs architecture.md, MODULES-AND-FEATURES.md, decisions.md, MEMORY [110]] (completed: 2026-05-30)`
...
-   `[done] P0: Purchase Invoice — Duplicate Inventory Transaction Prevention [lockForUpdate() on post; purchase_invoices/show Post form single-submit; migration add purchase_invoice_line_id FK + unique on inventory_transactions with backfill; idempotent createInventoryTransaction + processPurchaseTransaction; PurchaseInvoiceLine::inventoryTransactions(); ReportPurchaseInvoiceInventoryDuplicates command (Kernel registered); Feature test PurchaseInvoiceInventoryTransactionTest; phpunit.xml DB_DATABASE=sarang_erp_test to isolate RefreshDatabase from dev DB; docs/action-plans/inventory-transaction-deduplication-prevention.md, docs/decisions.md, MEMORY.md [095]] (completed: 2026-03-31)`
...
-   `[done] P0: Sales Order Approval Workflow Fix & UI Enhancement [Fixed Sales Order approval failures caused by missing approval records in sales_order_approvals table, implemented auto-recovery mechanism in SalesService::approveSalesOrder() to automatically create missing approval records, created FixSalesOrderApproval artisan command for bulk-fixing existing Sales Orders (--all flag or specific orderNo), created EnsureOfficerRole command to ensure "officer" role exists in both Spatie Permission system (roles table) and approval workflow system (user_roles table), added fix route /sales-orders/fix-approval/{orderNo} for ad-hoc fixes, enhanced Sales Order show page to display Item Code and Item Name columns from inventory_items table via relationship, updated SalesOrderController::show() to eager load inventoryItem relationship, fixed custom App\Console\Kernel to properly register commands, comprehensive browser testing validation confirming approval workflow and UI enhancements work correctly, app/Services/SalesService.php, app/Console/Commands/FixSalesOrderApproval.php, app/Console/Commands/EnsureOfficerRole.php, app/Console/Kernel.php, app/Http/Controllers/SalesOrderController.php, resources/views/sales_orders/show.blade.php, routes/web/orders.php, PRODUCTION_DEPLOYMENT.md] (completed: 2026-02-09)`
-   `[done] P1: Approval Workflow Admin UI Implementation [Created comprehensive admin UI for managing approval workflows with full CRUD operations, workflow step configuration, and threshold management, ApprovalWorkflowController with index/create/store/show/edit/update/destroy methods and threshold management (storeThreshold/updateThreshold/destroyThreshold), AdminLTE views with DataTables integration, modal-based threshold management, dynamic workflow step addition/removal, threshold overlap validation, professional UI styling consistent with ERP admin features, routes added in routes/web/admin.php with admin.approval-workflows permission, comprehensive browser testing validation confirming all features work correctly, app/Http/Controllers/Admin/ApprovalWorkflowController.php, resources/views/admin/approval-workflows/*.blade.php, routes/web/admin.php] (completed: 2025-01-22)`
